...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($erro_sistema)) {
    ...
    if (empty($erros)) {
        try {
            ...
            registar_log('editar', "Garantia/contrato atualizado para o equipamento: {$equipamentoNome}.", $_SESSION['id_utilizador'] ?? null);
            header('Location: listar.php');
            exit;
        } catch (PDOException $err) {
            $erro_sistema = "Erro ao atualizar os dados: " . $err->getMessage();
            registar_log('erro', "Erro ao atualizar a garantia/contrato na base de dados.", $_SESSION['id_utilizador'] ?? null);
        }
    }
}
